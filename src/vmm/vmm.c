...
static struct vmm_cpu *vmm_setup_cpu(void)
{
  /* Enable SVM */
...
static void vmm_setup_npt(struct vmm_cpu *ctx)
{
...
struct vmm_cpu *vmm_setup_bsp(struct grr_handover *handover)
{
  struct vmm_cpu *ctx;
...
struct vmm_cpu *vmm_setup_ap(void)
{
  struct vmm_cpu *ctx;
...
static void *guest_pgwalk(uint64_t *pml4, uint64_t virt_addr)
{
